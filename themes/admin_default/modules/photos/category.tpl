    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h3 class="panel-title" style="float: left">
                <i class="fa fa-list"></i> {LANG.category_list}
            </h3>
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <tr id="group_{LOOP.category_id}">
                                <td class="text-center"><select id="id_weight_{LOOP.category_id}" onchange="nv_change_category('{LOOP.category_id}','weight');" class="form-control">
                                        <!-- BEGIN: weight -->
                                        <option value="{WEIGHT.w}"{WEIGHT.selected}>{WEIGHT.w}</option>
                                        <!-- END: weight -->
                                </select></td>
                                <td class="text-left"><a href="{LOOP.link}"> <strong>{LOOP.name}</strong>
                                </a> {LOOP.numsubcat}</td>
                                <td class="text-center"><select class="form-control" id="id_inhome_{LOOP.category_id}" onchange="nv_change_category('{LOOP.category_id}','inhome');">
                                        <!-- BEGIN: inhome -->
                                        <option value="{INHOME.value}"{INHOME.selected}>{INHOME.title}</option>
                                        <!-- END: inhome -->
                                </select></td>

                                <td class="text-center"><select class="form-control" id="id_status_{LOOP.category_id}" onchange="nv_change_category('{LOOP.category_id}','status');">
                                        <!-- BEGIN: status -->
                                        <option value="{STATUS.value}"{STATUS.selected}>{STATUS.title}</option>
                                        <!-- END: status -->
                                </select></td>

                                <td><select class="form-control" id="id_viewcat_{LOOP.category_id}" onchange="nv_change_category('{LOOP.category_id}','viewcat');">
                                        <!-- BEGIN: viewcat -->
                                        <option value="{VIEWCAT.key}"{VIEWCAT.selected}>{VIEWCAT.title}</option>
                                        <!-- END: viewcat -->
                                </select></td>
                                <td class="text-center"><select class="form-control" id="id_numlinks_{LOOP.category_id}" onchange="nv_change_category('{LOOP.category_id}','numlinks');">
                                        <!-- BEGIN: numlinks -->
                                        <option value="{NUMLINKS.key}"{NUMLINKS.selected}>{NUMLINKS.title}</option>
                                        <!-- END: numlinks -->
                                </select></td>
                                <td class="text-center"><a href="{LOOP.edit}" data-toggle="tooltip" title="{LANG.edit}" class="btn btn-primary"><i class="fa fa-pencil"></i></a> &nbsp;&nbsp; <a href="javascript:void(0);" onclick="delete_category('{LOOP.category_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger"><i class="fa fa-trash-o"></i></td>
                            </tr>
                            <!-- END: loop -->
                        </tbody>
                    </table>
