...
if (ACTION_METHOD == 'add' || ACTION_METHOD == 'edit') {
    ...
    if ($nv_Request->get_int('save', 'post') == 1) {
        ...
        $_nb = $db->query('SELECT COUNT(*) FROM ' . TABLE_PHOTO_NAME . '_album WHERE album_id != ' . $data['album_id'] . ' AND folder=' . $db->quote($check_path . '/' . $data['folder']))
            ->fetchColumn();
        if (!empty($_nb)) {
