...
if ( ! empty( $catid ) )
{
	$channel['title'] = $module_info['custom_title'] . ' - ' . $global_photo_cat[$catid]['title'];
    ...
    $channel['description'] = $global_photo_cat[$catid]['description'];
...
if ( $module_info['rss'] )
{
    ...
    while ( list( $id, $listcatid, $publtime, $title, $alias, $hometext, $homeimgfile ) = $result->fetch( 3 ) )
    {
        ...
        $catalias = $global_photo_cat[$catid_i]['alias'];
